...
def getRange(s):
    if (s.find(":") == -1):
        ...
        index = s.index(":")
        return (int(s[index+1:]), int(s[:index]))
...
if __name__ == "__main__":
    ...
    with open('./result_list.json') as f_json:
    ...
    for page in book:
        ...
        for address in page['registers']:
            ...
            output.write("\t\t{: >5} {: >22} {: >20} {: >20}\tDescription\n".format(*data))
            for bit in address['bits']:
                index = getRange(bit['bit'])
                length = math.ceil(float((index[1]+1-index[0]))/4)
                val = "0x%0{0}X".format(length) % (int(VALUE[index[0]:index[1]+1][::-1],2))
                data = [bit['bit'],bit['name'], val, "0x%0{0}X".format(length) %bit['default']]
                output.write("\t\t{: >5} {: >22} {: >20} {: >20}".format(*data))
                string = u"\t%s\n"%bit['description']
                output.write(string.encode('utf8'))
    output.close()
